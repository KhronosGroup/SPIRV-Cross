...
uint32_t CompilerMSL::add_interface_struct(StorageClass storage, uint32_t vtx_binding)
{
	...
	ib_type.is_packed = (execution.model == ExecutionModelVertex &&
						 ...
						 var_dec.binding != msl_options.vtx_attr_stage_in_binding);
...
{
	statement("// Autogenerated by spirv_cross.");
	statement("");
		...
		if (is_builtin) {
			switch (builtin) {
				...
				case BuiltInPointSize:		// Must output only if really rendering points
					return msl_options.is_rendering_points ? (string(" [[") + builtin_qualifier(builtin) + "]]") : "";
...
string CompilerMSL::entry_point_args(bool append_comma)
{
	...
	for (uint32_t var_id : stage_in_var_ids)
	{
		...
		bool use_stage_in = (execution.model != ExecutionModelVertex || dec.binding == msl_options.vtx_attr_stage_in_binding);
