...
void CompilerMSL::build_implicit_builtins()
{
    ...
    bool need_vertex_params = capture_output_to_buffer && get_execution_model() == ExecutionModelVertex && !vertex_shader_is_kernel();
    ...
    !capture_output_to_buffer && !get_is_rasterization_disabled() && !active_output_builtins.get(BuiltInPosition) && !msl_options.for_mesh_pipeline;
...
string CompilerMSL::member_attribute_qualifier(const SPIRType &type, uint32_t index)
{
    ...
    if (((execution.model == ExecutionModelVertex && !vertex_shader_is_kernel()) || is_tese_shader() || (execution.model == ExecutionModelGeometry)) &&
