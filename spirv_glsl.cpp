...
void CompilerGLSL::emit_header()
{
	...
	// Library modules have no entry point. The emitted GLSL is meant to be included or appended
	// rather than compiled standalone, so we do not emit version or extension directives in that
	// case. That is the responsability of the application including the resulting glsl file. Note,
	// this may also require the user to provide SPIRV_CROSS_* definitions.
	if (ir.is_library_module)
	{
		statement("// Library generated by SPIRV-Cross. Requires #version ", options.version,
		          options.es && options.version > 100 ? " es" : "", " or later.");
		statement("");
		return;
	}
