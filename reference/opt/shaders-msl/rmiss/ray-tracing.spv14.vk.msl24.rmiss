...
{
    float3 _33 = float3(float(spvRayContext.launchId.x) / float(spvRayContext.launchSize.x), float(spvRayContext.launchId.y) / float(spvRayContext.launchSize.y), 1.0);
    {
        ...
        *reinterpret_cast<thread float*>(&reinterpret_cast<thread spvRayDataStorage*>(spvRayData)->payload) = (*reinterpret_cast<thread float*>(&reinterpret_cast<thread spvRayDataStorage*>(spvRayData)->payload));
        ...
        spvRayContext.worldRayOrigin = _33;
        spvRayContext.worldRayDirection = float3(0.0, 0.0, -1.0);
        ...
        spvRayQuery.reset(ray(_33, float3(0.0, 0.0, -1.0), 0.0, 1000.0), as, (255u & 255u), spvMakeIntersectionParams((0u | spvRayState.pipelineFlags)));
