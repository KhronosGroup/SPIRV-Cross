...
{
    main0_in out;
    out.a0 = unpack_unorm4x8_to_float(reinterpret_cast<const device uint&>(data0.a0));
    out.a1 = float4(float3(data0.a1).bgr, 1);
    out.a3 = max(float4(reinterpret_cast<const device packed_short4&>(data1.a3)) * (1.f / 32767), -1.f);
    out.a4 = unpack_unorm10a2_to_float(reinterpret_cast<const device uint&>(data2.a4));
    out.a5 = float4(spvLoadVertexRGB9E5Float(reinterpret_cast<const device uint&>(data2.a5)), 1);
    out.a6 = float4(float3(spvLoadVertexRG11B10Half(reinterpret_cast<const device uint&>(data2.a6))), 1);
