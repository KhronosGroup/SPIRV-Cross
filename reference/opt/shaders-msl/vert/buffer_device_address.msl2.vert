...
{
    main0_out out = {};
    out.gl_Position = registers.view_projection * float4((registers.references->buffers[int(gl_InstanceIndex)]->positions[int(gl_VertexIndex)] * 2.5) + ((float2(float((int(gl_InstanceIndex) - 8 * (int(gl_InstanceIndex) / 8) + 8) - 8 * ((int(gl_InstanceIndex) - 8 * (int(gl_InstanceIndex) / 8) + 8) / 8)), float(int(gl_InstanceIndex) / 8)) - float2(3.5)) * 3.0), 0.0, 1.0);
    int _82 = (int(gl_VertexIndex) - 16 * (int(gl_VertexIndex) / 16) + 16) - 16 * ((int(gl_VertexIndex) - 16 * (int(gl_VertexIndex) / 16) + 16) / 16);
