...
[[visible]] void main_mvkClosestHit(thread void* spvRayData, thread spvRayTracingContext& spvRayContext, thread uint& spvRayAction, thread spvRayTracingState& spvRayState)
{
    ...
    builtins = _83;
    (*reinterpret_cast<thread float4*>(&reinterpret_cast<thread spvRayDataStorage*>(spvRayData)->payload)) = float4(*reinterpret_cast<thread const float2*>(&spvRayContext.hitAttribute), _83, 1.0);
}
