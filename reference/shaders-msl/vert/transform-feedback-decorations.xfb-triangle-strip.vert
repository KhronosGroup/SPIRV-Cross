...
kernel void main0(uint3 gl_GlobalInvocationID [[thread_position_in_grid]], uint3 spvStageInputSize [[grid_size]], device atomic_uint* spvXfbCounter1 [[buffer(17)]], device spvXfbBuffer1* spvXfb1 [[buffer(13)]], device atomic_uint* spvXfbCounter2 [[buffer(18)]], device spvXfbBuffer2* spvXfb2 [[buffer(14)]], device atomic_uint* spvXfbCounter3 [[buffer(19)]], device spvXfbBuffer3* spvXfb3 [[buffer(15)]])
{
    ...
    uint spvXfbIndex = 3 * gl_GlobalInvocationID.y * subsat(spvStageInputSize.x, 2u) + 3 * gl_GlobalInvocationID.x;
    ...
    if (gl_GlobalInvocationID.x + 2 < spvStageInputSize.x)
        spvXfb1[spvXfbIndex] = spvXfbOutput1;
    if (gl_GlobalInvocationID.x != 0 && gl_GlobalInvocationID.x != spvStageInputSize.x - 1u)
        spvXfb1[spvXfbIndex - 1u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput1;
    if (gl_GlobalInvocationID.x > 1)
        spvXfb1[spvXfbIndex - 4u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput1;
    ...
    if (gl_GlobalInvocationID.x != 0 && gl_GlobalInvocationID.x != spvStageInputSize.x - 1u)
        spvXfb2[spvXfbIndex - 1u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput2;
    if (gl_GlobalInvocationID.x > 1)
        spvXfb2[spvXfbIndex - 4u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput2;
    ...
    if (gl_GlobalInvocationID.x != 0 && gl_GlobalInvocationID.x != spvStageInputSize.x - 1u)
        spvXfb3[spvXfbIndex - 1u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput3;
    if (gl_GlobalInvocationID.x > 1)
        spvXfb3[spvXfbIndex - 4u - (gl_GlobalInvocationID.x & 1u)] = spvXfbOutput3;
