...
void updatePayload(thread float4 (&payload)[2][3], thread void* spvRayData, thread spvRayTracingContext& spvRayContext, thread uint& spvRayAction, thread spvRayTracingState& spvRayState)
{
    payload[1][2].x += 1.0;
}
...
{
    float4 param[2][3];
    spvArrayCopyFromStackToStack(param, reinterpret_cast<thread spvRayDataWithPayload_29*>(spvRayData)->payload);
    updatePayload(param, spvRayData, spvRayContext, spvRayAction, spvRayState);
    spvArrayCopyFromStackToStack(reinterpret_cast<thread spvRayDataWithPayload_29*>(spvRayData)->payload, param);
