...
fragment main0_out main0(main0_in in [[stage_in]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_TranslucentBasePass& TranslucentBasePass [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<float> TranslucentBasePass_Shared_Fog_IntegratedLightScattering [[texture(0)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], float4 gl_FragCoord [[position]], bool gl_FrontFacing [[front_facing]], uint gl_SampleMaskIn [[sample_mask]])
{
    main0_out out = {};
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            bool3 _242 = float3(fract(dot(_142, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5);
            _245 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), float3(_242.x ? float3(1.0).x : float3(0.0).x, _242.y ? float3(1.0).y : float3(0.0).y, _242.z ? float3(1.0).z : float3(0.0).z));
        }
        else
        {
            _245 = _216;
    ...
    if (View.View_NumSceneColorMSAASamples > 1)
    {
        ...
        _269 = gl_SampleMaskIn[0] & 15u;
    }
    else
    {
        _268 = _256;
        _269 = gl_SampleMaskIn[0];
    }
