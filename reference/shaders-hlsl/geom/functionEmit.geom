...
void emitPoint(float4 _point, float3 color, inout TriangleStream<SPIRV_Cross_Output> triStream)
{
    gColor = color;
    gl_Position = _point;
    {
...
void emitTriangle(float4 center, float3 color, inout TriangleStream<SPIRV_Cross_Output> triStream)
{
    float4 param = center + float4(-0.100000001490116119384765625f, -0.100000001490116119384765625f, 0.0f, 0.0f);
    float3 param_1 = color;
    emitPoint(param, param_1, triStream);
...
void functionThatIndirectlyEmits(inout TriangleStream<SPIRV_Cross_Output> triStream)
{
    float4 param = float4(1.0f, 2.0f, 3.0f, 4.0f);
    float3 param_1 = float3(1.0f, 0.0f, 1.0f);
    emitTriangle(param, param_1, triStream);
}
...
void geom_main(point SPIRV_Cross_Input stage_input[1], inout TriangleStream<SPIRV_Cross_Output> triStream)
{
    ...
    emitTriangle(param_3, param_4, triStream);
    functionThatIndirectlyEmits(triStream);
}
